...
def split_dataset_by_vaga(df: pd.DataFrame, test_size=0.15, val_size=0.1, random_state=1993):
    ...
    logger.success(f"[Split] Feito! Tamanhos: train={len(df_train_final)}, val={len(df_val_final)}, test={len(df_test)}")

    return df_train_final.reset_index(drop=True), df_val_final.reset_index(drop=True), df_test.reset_index(drop=True)

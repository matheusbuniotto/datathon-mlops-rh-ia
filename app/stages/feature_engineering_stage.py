...
def get_preprocessing_pipeline():
    ...
    ohe_sparse_param = (
        {"sparse_output": True} if sklearn_version >= (1, 2) else {"sparse": True}
    )
    ...
    ordinal_pipeline = Pipeline(
        [
            (
                "encoder",
                OrdinalEncoder(
                    categories=[ordinais[col] for col in ordinais],
    ...
    categorical_pipeline = Pipeline(
        [
            (
                "encoder",
                OneHotEncoder(
                    handle_unknown="ignore", min_frequency=100, **ohe_sparse_param
                ),
            )
        ]
    )
    ...
    emb_cv_pipeline = Pipeline([("pca", PCA(n_components=75))])

    emb_vaga_pipeline = Pipeline([("pca", PCA(n_components=75))])
...
def get_feature_names_from_pipeline(pipeline, input_df):
    try:
            ...
            if name == "ordinal":
                ...
                if hasattr(encoder, "get_feature_names_out"):
                    ...
                    for i, col in enumerate(columns):
                        if hasattr(encoder, "categories_") and i < len(
                            encoder.categories_
                        ):
        ...
        logger.info(
            f"[Features] Extracted {len(feature_names)} feature names from pipeline"
        )
...
def calculate_area_similarity(df):
    ...
    def fuzzy_similarity(row):
        ...
        if candidato_area in ["nan", "indefinido", ""] or vaga_area in [
            "nan",
            "indefinido",
            "",
        ]:
...
def apply_feature_pipeline(df_train, df_val, df_test):
    ...
    df_train["candidato_area_atuacao"] = clean_area_atuacao(
        df_train, "candidato_area_atuacao"
    )
    df_val["candidato_area_atuacao"] = clean_area_atuacao(
        df_val, "candidato_area_atuacao"
    )
    df_test["candidato_area_atuacao"] = clean_area_atuacao(
        df_test, "candidato_area_atuacao"
    )

    df_train["vaga_areas_atuacao_clean"] = clean_area_atuacao(
        df_train, "vaga_areas_atuacao"
    )
    df_val["vaga_areas_atuacao_clean"] = clean_area_atuacao(
        df_val, "vaga_areas_atuacao"
    )
    df_test["vaga_areas_atuacao_clean"] = clean_area_atuacao(
        df_test, "vaga_areas_atuacao"
    )
    ...
    if feature_names:
        ...
        ordinal_features = [
            ...
            if any(
                nivel in f
                for nivel in ["nivel_profissional", "nivel_academico", "nivel_ingles"]
            )
        ]
        numerical_features = [
            f
            for f in feature_names
            if f in ["vaga_sap", "area_similarity"] or f.startswith("emb_cv_pca_")
        ]
        categorical_features = [
            f
            for f in feature_names
            if f not in ordinal_features and f not in numerical_features
        ]

        logger.info(
            f"[Features] Ordinal Features ({len(ordinal_features)}): {ordinal_features}"
        )
        logger.info(
            f"[Features] Numerical Features ({len(numerical_features)}): {numerical_features[:10] + ['...'] if len(numerical_features) > 10 else numerical_features}"
        )
        ...
        if categorical_features and len(categorical_features) <= 10:
            ...
            logger.info(
                f"[Features] ... and {len(categorical_features) - 10} more categorical features"
            )

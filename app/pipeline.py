...
def run_pipeline():
    ...
    df_prospects.to_parquet(
        os.path.join(DATA_PROCESSED, "prospects.parquet"), index=False
    )
    ...
    df_ = pd.read_parquet(
        os.path.join(EMBEDDING_PATH_SAVE, "combined_embeddings.parquet")
    )
    df_rank = prepare_ranking_dataset(df_)
    df_rank.to_parquet("data/processed/rank_ready.parquet", index=False)

    logger.success(
        "[Pipeline] Dataset com target de rankeamento salvo em data/processed/rank_ready.parquet"
    )

    print(
        "[SUCCESS] Pipeline executado com sucesso. Arquivo salvo em data/processed/merged.parquet"
    )
    return df_merged

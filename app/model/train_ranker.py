...
def load_feature_names(path):
    ...
    try:
        ...
        if os.path.exists(json_path):
            ...
            logger.success(
                f"[LightGBM] {len(feature_names)} nomes de features carregados do JSON"
            )
        ...
        if os.path.exists(npy_path):
            ...
            logger.success(
                f"[LightGBM] {len(feature_names)} nomes de features carregados do numpy"
            )
            return feature_names
...
def log_feature_info(X_train, feature_names=None):
    ...
    logger.info(
        f"[LightGBM] Esparsidade: {1 - X_train.nnz / (X_train.shape[0] * X_train.shape[1]):.2%}"
    )
    ...
    if feature_names and len(feature_names) == X_train.shape[1]:
        ...
        for feature in feature_names:
            if any(
                col in feature
                for col in ["nivel_profissional", "nivel_academico", "nivel_ingles"]
            ):
        ...
        if numerical_features:
            logger.info(
                f"[LightGBM] 🔢 Features Numéricas ({len(numerical_features)}):"
            )
        ...
        if categorical_features:
            logger.info(
                f"[LightGBM] 🏷️  Features Categóricas ({len(categorical_features)}):"
            )
            ...
            for prefix, features in cat_groups.items():
                ...
                if len(features) <= 5:
                    ...
                    for feat in features[:3]:
                        logger.info(f"[LightGBM]     - {feat}")
                    logger.info(
                        f"[LightGBM]     ... e mais {len(features) - 3} valores"
                    )
        ...
        try:
            ...
            logger.info(
                f"[LightGBM] Lista completa de features salva em {features_log_path}"
            )
        ...
        if feature_names is None:
            ...
            logger.error(
                f"[LightGBM] ❌ Incompatibilidade: {len(feature_names)} nomes vs {X_train.shape[1]} features"
            )
...
def load_inputs(path):
    ...
    logger.info(
        f"[LightGBM] Dados carregados: X_train{X_train.shape}, y_train{y_train.shape}, group_train{group_train.shape}"
    )
    logger.info(f"[LightGBM] Primeiros 10 grupos: {group_train[:10]}")
    logger.info(
        f"[LightGBM] Total de grupos: {len(group_train)}, Total de amostras: {group_train.sum()}"
    )
...
if __name__ == "__main__":
    ...
    if feature_names:
        ...
        logger.info(
            "[LightGBM] Nomes das features salvos em app/model/feature_names.json"
        )
        ...
        logger.warning(
            "[LightGBM] ⚠️  Modelo treinado, mas nomes das features não disponíveis"
        )

...
def load_inputs(dataset_type):
    ...
    logger.info(f"[Eval] Dados {dataset_type} carregados: {X.shape}, {y.shape}, {group.shape}")
    return X, y, group
...
def evaluate_model(model, X, y, group, dataset_name, k=5, relevance_threshold=0):
    ...
    logger.info(f"[Eval {dataset_name.upper()}] Grupos válidos: {valid_groups}/{total_groups}")
...
def development_evaluation(model_path, k=5, relevance_threshold=0):
    ...
    return evaluate_model(model, X_val, y_val, group_val, "validation", k, relevance_threshold)
...
def final_test_evaluation(model_path, k=5, relevance_threshold=0):
    ...
    return evaluate_model(model, X_test, y_test, group_test, "test", k, relevance_threshold)

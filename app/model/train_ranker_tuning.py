...
def load_inputs(path, split):
    ...
    logger.info(
        f"[LightGBM] Dados {split} carregados: {X.shape}, {y.shape}, {group.shape}"
    )
    return X, y, group


def train_lgbm_ranker_with_validation(
    X_train, y_train, group_train, X_val, y_val, group_val, params
):
...
def hyperparameter_tuning(
    ...
    study.optimize(
        lambda trial: objective(
            trial, X_train, y_train, group_train, X_val, y_val, group_val
        ),
        n_trials=n_trials,
    )
...
if __name__ == "__main__":
    ...
    if optimization_method == "optuna":
        ...
        best_params = hyperparameter_tuning(
            X_train, y_train, group_train, X_val, y_val, group_val, n_trials=30
        )

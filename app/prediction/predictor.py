...
def detect_data_drift(df_new: pd.DataFrame, reference_profile_path: str):
    ...
    for feature, profile in reference_profile["numerical"].items():
        if feature in df_new.columns:
            p_value = ks_2samp(
                df_new[feature],
                np.random.normal(profile["mean"], profile["std"], len(df_new[feature])),
            )[1]
            DATA_DRIFT_GAUGE.labels(feature=feature, test_type="ks").set(p_value)
            if p_value < 0.05:
                logger.warning(
                    f"Data drift detected for numerical feature '{feature}' (p-value: {p_value:.4f})"
                )
    ...
    for feature, profile in reference_profile["categorical"].items():
        if feature in df_new.columns:
            ...
            observed_counts = (
                df_new[feature].value_counts().reindex(profile.keys(), fill_value=0)
            )
            ...
            if p_value < 0.05:
                logger.warning(
                    f"Data drift detected for categorical feature '{feature}' (p-value: {p_value:.4f})"
                )


def predict_rank_for_vaga(
    ...
    try:
        ...
        df_vaga["candidato_area_atuacao"] = clean_area_atuacao(
            df_vaga, "candidato_area_atuacao"
        )
        df_vaga["vaga_areas_atuacao_clean"] = clean_area_atuacao(
            df_vaga, "vaga_areas_atuacao"
        )
        ...
        reference_profile_path = os.path.join(
            project_root, "data/monitoring/reference_profile.json"
        )
        detect_data_drift(
            pd.DataFrame(X_processed, columns=pipe.get_feature_names_out()),
        ...
        return {"vaga_id": vaga_id, "candidatos": top_candidates}
